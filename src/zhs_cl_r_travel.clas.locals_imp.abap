CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS setInitialStatus FOR DETERMINE ON MODIFY
      IMPORTING
        keys FOR Travel~setInitialStatus.
    ...
    METHODS is_update_granted
      ...
      RETURNING
        VALUE(update_granted) TYPE abap_bool.
    ...
    METHODS is_delete_granted
      IMPORTING
        ...
        overall_status        TYPE /dmo/overall_status
      RETURNING
        VALUE(delete_granted) TYPE abap_bool.

    METHODS is_create_granted
      RETURNING
        VALUE(create_granted) TYPE abap_bool.
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zhs_r_travel IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF zhs_r_travel IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
           WITH VALUE #( FOR key IN keys
                ( %tky = key-%tky TravelStatus = travel_status-accepted ) )
      FAILED failed
      REPORTED reported.
    ...
    MODIFY ENTITIES OF zhs_r_travel IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
           WITH VALUE #( FOR key IN keys
             ( %tky = key-%tky TravelStatus = travel_status-canceled ) )
      FAILED failed
      REPORTED reported.
    ...
    ENTITY Travel
      UPDATE
        ...
        WITH VALUE #( FOR travel IN travels
            ( %tky = travel-%tky TravelStatus = travel_status-open ) )
    ...
    ENTITY Travel
      UPDATE
        FROM VALUE #( FOR travel IN travels INDEX INTO i (
          %tky = travel-%tky TravelID = max_travelid + i %control-TravelID = if_abap_behv=>mk-on ) )
    REPORTED DATA(update_reported).
  ...
  METHOD validateAgency.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      APPEND VALUE #( %tky = travel-%tky %state_area = 'VALIDATE_AGENCY' )
        TO reported-travel.
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky = travel-%tky
                        ...
                        %msg = NEW zhs_cl_rap_message(
                                      severity = if_abap_behv_message=>severity-error
                                      textid   = zhs_cl_rap_message=>agency_unknown
  ...
  METHOD validateCustomer.
    ...
    LOOP AT travels INTO DATA(travel).
      " Clear state messages that might exist
      APPEND VALUE #( %tky = travel-%tky %state_area = 'VALIDATE_CUSTOMER' )
        TO reported-travel.
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky = travel-%tky
                         ...
                         %msg = NEW zhs_cl_rap_message(
                                        severity   = if_abap_behv_message=>severity-error
                                        textid     = zhs_cl_rap_message=>customer_unknown
  ...
  METHOD validateDates.
    ...
    LOOP AT travels INTO DATA(travel).
      " Clear state messages that might exist
      APPEND VALUE #( %tky = travel-%tky %state_area = 'VALIDATE_DATES' )
        TO reported-travel.
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky = travel-%tky
                        ...
                        %msg = NEW zhs_cl_rap_message(
                                         severity  = if_abap_behv_message=>severity-error
                                         textid    = zhs_cl_rap_message=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky = travel-%tky
                        ...
                        %msg = NEW zhs_cl_rap_message(
                                       severity  = if_abap_behv_message=>severity-error
                                       textid    = zhs_cl_rap_message=>begin_date_before_system_date
                                       begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      READ TABLE travels_before_image INTO DATA(travel_before_image) WITH KEY travel_uuid = travel-TravelUUID
        BINARY SEARCH.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky = travel-%tky
                            %msg = NEW zhs_cl_rap_message(
                                             severity = if_abap_behv_message=>severity-error
                                             textid = zhs_cl_rap_message=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
  ...
  METHOD is_create_granted.
    AUTHORITY-CHECK OBJECT 'ZHS_AO_ST'
      ID 'ZHS_AF_ST' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZHS_AO_ST'
        ID 'ZHS_AF_ST' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZHS_AO_ST'
        ID 'ZHS_AF_ST' DUMMY
